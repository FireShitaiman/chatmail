...
| ファイル | 役割 |
|---|---|
| `main.js` | Electron 起動・ウィンドウ管理のみ |
| `gmail.js` | Gmail API・OAuth2・全 IPC ハンドラー |
| `preload.js` | `window.electronAPI` を contextBridge で公開 |
| `index.html` | HTML 構造のみ |
| `styles.css` | 全スタイル |
| `renderer.js` | レンダラー側の全 JavaScript |
...
| チャネル | 実装 | 内容 |
|---|---|---|
| `gmail:get-config` | gmail.js | 認証状態・clientId を返す |
| `gmail:save-config` | gmail.js | clientId / clientSecret を保存 |
| `gmail:authenticate` | gmail.js | OAuth2 フロー（ブラウザ → port 3000 コールバック） |
| `gmail:signout` | gmail.js | トークン削除 |
| `gmail:fetch-threads` | gmail.js | 受信トレイ上位 20 スレッドを取得 |
| `gmail:fetch-messages` | gmail.js | スレッドの全メッセージを取得 |
| `gmail:send` | gmail.js | メール送信（スレッド返信） |
| `open-external` | gmail.js | ブラウザで URL を開く |
